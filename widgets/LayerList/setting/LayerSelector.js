...
define([
  ...
  domClass, domStyle, Select, topic) {
  ...
  return declare([_WidgetBase, _TemplatedMixin], {
    ...
    postCreate: function() {
      //  var publishEvent = dojo.subscribe("group-data", this.layerCreation.bind(this));
        this.layers = this.operLayerInfos.getLayerInfoArray();
         var publishEvent = dojo.subscribe("groupsDone", this.layerCreation.bind(this));
 array.forEach(this.operLayerInfos.getLayerInfoArray(), function(layerInfo) {
     ...
     layerCreation: function(data){
         this.finalGroupFromSetting.push(data);
         array.forEach(this.layers, function(layerInfoFromLayers, index) {   
                  this.layers[index].groupname = this.finalGroupFromSetting[0].groupname;
       },this);
         
         if(data.layers == this.finalGroupFromSetting.length){
             this.selectDropDownCreation();
         }
     } ,
      selectDropDownCreation : function(){
          if(this.firstTime ==1){
           array.forEach(this.operLayerInfos.getLayerInfoArray(), function(layerInfo, index) {          
         var divElement = document.getElementById(layerInfo.title.replace(/ /g,"-"));
          var selectList = document.createElement("select");
         selectList.id = "mySelect"+index;       
         divElement.appendChild(selectList);

        //Create and append the options
        for (var i = 0; i < this.finalGroupFromSetting.length; i++) {
            var option = document.createElement("option");
            option.value = this.finalGroupFromSetting[i].groupname;
            option.text = this.finalGroupFromSetting[i].groupname;
            selectList.appendChild(option);
        }
        this.own(on(selectList,
        'change',
        lang.hitch(this,
          this._onLayerSelectionClick,
                  layerInfo,selectList))); 
          ...
          this.firstTime = 2; 
               ...
               array.forEach(this.operLayerInfos.getLayerInfoArray(), function(layerInfo, index) {  
               var x = document.getElementById("mySelect"+index);
//                   console.log(this.lastValue+"#########"+this.finalGroupFromSetting[0].layers)
//                   if(this.lastValue > this.finalGroupFromSetting[0].layers){
//                x.remove(x.length-1);
//                        }
//                   else{
//                       var option = document.createElement("option");
//                       option.text = this.finalGroupFromSetting[this.finalGroupFromSetting.length -1].groupname ;
//                       x.add(option);
//                   }
             // this.firstTime =1;
              //this.selectDropDownCreation();
         ...
         // this.finalGroupFromSetting = []
      },

    drawListNode: function(layerInfo, level, toTableNode, groupName) {
      var nodeAndSubNode;
      if (layerInfo.newSubLayers.length === 0) {
        //addLayerNode
        nodeAndSubNode = this.addLayerNode(layerInfo, level, toTableNode, groupName);
    ...
    addLayerNode: function(layerInfo, level, toTableNode, groupName) {
    ...
    var layerTitleDivIdClassForGroup = 'layer-title-div-' + layerInfo.title;
        var optionArray = [];
      var  selectDiv = domConstruct.create('div',{
           'class' : layerTitleDivIdClassForGroup + ' select-option-group label-infoForGroup div-content',
          'id':layerInfo.title.replace(/ /g,"-")
        },layerTitleTdNode); 
       ...
       array.forEach(this.operLayerInfos.getLayerInfoArray(), function(layerInfo, index) {    
         ...
         optionArray.push({'label':layerInfo.title,'value':layerInfo.title});
    ...
    _changeSelectedLayerRow: function(layerTrNode) {
      ...
      _onLayerSelectionClick: function(layerInfo,valueofSelect) {
          array.forEach(this.layers, function(layerInfoFromLayers, index) {     
              if(layerInfoFromLayers == layerInfo){
                  this.layers[index] = {'groupname':valueofSelect.value};
              }
       },this);
        
//          this.count = this.count + 1;
//          console.log(123,this.finalGroup);
//          var tempArrayForGroups = [];
//          if(this.finalGroup.length == 0){
//          this.finalGroup.push({'groupname':valueofSelect.value,'layers':layerInfo}); 
//          }
//          else{
//          array.forEach(this.finalGroup, function(groupItem, index) {    
//               console.log(123123,groupItem.layers,layerInfo);
//                if(groupItem.layers == layerInfo){
//                    groupItem.groupname = valueofSelect.value;
//                }
//       },this);         
//                this.finalGroup.push({'groupname':valueofSelect.value,'layers':layerInfo});   
//        
...
//         dojo.publish("globalEvents",{'groupname':valueofSelect.value,'layers':[layerInfo]}); 
    ...
    getLayerOptions: function() {
      ...
      for(var child in this._displayStateStorage) {
        if(this._displayStateStorage.hasOwnProperty(child) &&
          ...
          layerOptions[child] = {display: this._displayStateStorage[child].displayCK.checked,groupname: this.layers[i].groupname};
          
        }
          i = i+1;
      }
      return layerOptions;
      //  return this.finalGroupFromSetting;
    }
